package org.framework.integration.example.web.mvc.boostrap;

import lombok.AccessLevel;
import lombok.NoArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.framework.integration.example.web.mvc.common.annotations.Log;
import org.framework.integration.example.web.mvc.common.annotations.LogGroup;
...
public class OperationLogAuditBootstrapService implements BootstrapService {
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        log.info("开始检查controller中的审计日志注解");
        ...
        methodList.stream().filter(m -> !EXCLUDE_CLASS_LIST.contains(m.getDeclaringClass().getSimpleName())).forEach(m -> {
            Assert.isTrue(m.getDeclaringClass().isAnnotationPresent(LogGroup.class), m.getDeclaringClass() + "的类缺少Api注释");
            Assert.isTrue(m.isAnnotationPresent(Log.class), MethodUtil.getMethodDescription(m) + "的方法缺少ApiResource注释");
        });
